;;;; vp8-decoder.lisp — a complete VP8 video decoder (RFC 6386): key frames
...
;;;; transforms, loop-filter kernels) is webp-pure's; this file adds what a
;;;; still image never needed: the inter-frame header, mode/MV decoding,
...
(defstruct (vp8-decoder (:conc-name vd-))
...
(defun vp8-frame-info (bytes &key (start 0) (end (length bytes)))
  "Peek at a frame: (values key-frame-p show-p width height version)."
...
(defun vp8-decode-frame (vd bytes &key (start 0) (end (length bytes)) timestamp)
  ...
  (when (= start end)
    ...
    (return-from vp8-decode-frame (values (vd-last-output vd) (and (vd-last-output vd) t))))
