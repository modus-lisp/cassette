...
| Codecs | Video is [`reel`](../reel)'s: VP8, VP9, H.264 (Baseline/Main/High, P and B slices), MPEG-1/2, MPEG-4 Part 2, Theora and FFV1.  `next-video-frame` picks the decoder off the track and the caller does not have to care which.  H.264 is read ahead in batches (`*h264-read-ahead*`) and decoded on several cores at once where the stream allows it, which today means all-intra.  A codec nothing here decodes is named in `player-unsupported` rather than hidden, and the file still opens and plays whatever else it has.  Audio is [`reed`](../reed)'s: Opus, Vorbis, AAC and MPEG audio Layer II. |
