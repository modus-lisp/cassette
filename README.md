...
| Piece | State |
| ----- | ----- |
| VP8 decoder (in `reel`, asserted here) | **Bit-exact with ffmpeg/libvpx** on the libvpx-encoded vectors (synthetic clips with alt-ref, 2/4 token partitions, error-resilient mode, split MVs) and on Big Buck Bunny 640x360. About 350 fps at 640x360 on one core. **Open:** the two W3C WPT clips (`wpt-test`, `wpt-circles`, an older encoder) differ slightly — ±1 on skipped ZEROMV macroblocks, larger on one B_PRED macroblock inside an inter frame; the partition parse stays in sync, so it is a reconstruction detail, not a desync. |
...
| Audio | Opus, Vorbis, FLAC, AC-3, AAC and MPEG audio Layer II through `reed`.  Vorbis is the audio half of the *original* WebM pairing — a `.webm` from before about 2013 is VP8 and Vorbis — and its three configuration headers arrive Xiph-laced in Matroska's CodecPrivate and as the head of their own logical stream in Ogg, which is two code paths sharing nothing (`inspect/test-vorbis.lisp`). |
...
| Codecs | Video is [`reel`](../reel)'s: VP8, VP9, H.264 (Baseline/Main/High, P and B slices), MPEG-1/2, MPEG-4 Part 2, Theora and FFV1.  `next-video-frame` picks the decoder off the track and the caller does not have to care which.  H.264 is read ahead in batches (`*h264-read-ahead*`) and decoded on several cores at once where the stream allows it, which today means all-intra.  A codec nothing here decodes is named in `player-unsupported` rather than hidden, and the file still opens and plays whatever else it has.  Audio is [`reed`](../reed)'s: Opus, Vorbis, FLAC, AC-3, AAC and MPEG audio Layer II.  AC-3 out of a program stream needs two things Matroska never asks for: private stream 1 is a bundle whose four-byte substream header is not audio, and its PES boundaries are not AC-3 frame boundaries (`inspect/test-ac3.lisp`). |
...
| Not done | An MP4 muxer (the muxer writes WebM only), and A/V pacing — the player is pull-model and the caller paces; see `warp-media` for a paced player on top of this. AV1 and HEVC are not decoded by `reel` and are named rather than guessed at. |
