...
(defun compare-vector (name)
  (let* ((path (format nil "vectors/~a.webm" name))
         ...
         (oracle (webm-pure::slurp-file oracle-path))
         (p (webm-pure:open-webm path :audio nil))
         (vt (webm-pure:player-video-track p))
         (w (webm-pure:track-width vt)) (h (webm-pure:track-height vt))
    ...
    (dotimes (i nframes)
      (let ((pic (webm-pure:next-video-frame p)))
        (unless pic
          ...
          (return))
        (let* ((yuv (webm-pure:picture->yuv420 pic))
               (off (* i frame-bytes))
               (maxd 0) (ndiff 0))
